...
# Create pyresparser config directory and file
config_path = os.path.join(os.path.expanduser('~'), '.pyresparser')
os.makedirs(config_path, exist_ok=True)

config_file = os.path.join(config_path, 'config.cfg')
if not os.path.exists(config_file):
    with open(config_file, 'w') as f:
        f.write("""[resume_parser]
experience_grammar = \[(\d+)\]
email=True
phone=True
mobile_number=True
skills=True
...
def run():
    ...
    link = '[@Developed by Shaista Afreen](https://www.linkedin.com/in/shaista-afreen-540616266/)'
